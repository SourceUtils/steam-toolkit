...
class DataBlockHeader {
    ...
    DataBlockHeader(RandomAccessFileWrapper raf) throws IOException {
        gcfRevision = raf.readULEInt(); blockCount = raf.readULEInt(); blockSize = raf.readULEInt();
        firstBlockOffset = raf.readULEInt(); blocksUsed = raf.readULEInt(); checksum = raf.readULEInt();
    }

    @Override
    public String toString() {
        int checked = 0; checked += DataUtils.updateChecksumAdd(blockCount); checked += DataUtils.updateChecksumAdd(blockSize);
        checked += DataUtils.updateChecksumAdd(firstBlockOffset); checked += DataUtils.updateChecksumAdd(blocksUsed);
